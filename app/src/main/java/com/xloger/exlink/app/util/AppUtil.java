...
//        qqWhiteUrl.add("qzs.qzone.qq.com");
        ...
        App tim = new App();
        tim.setAppName("TIM");
        tim.setPackageName("com.tencent.tim");
        Set<Rule> timRule = new HashSet<Rule>();
        timRule.add(new Rule("com.tencent.mobileqq.activity.QQBrowserDelegationActivity", "url"));
        tim.setRules(timRule);
        Set<String> timWhiteUrl = new HashSet<String>();
        timWhiteUrl.add("qzone.qq.com");
        timWhiteUrl.add("qun.qq.com");
        timWhiteUrl.add("jq.qq.com");
        timWhiteUrl.add("mqq.tenpay.com");
        timWhiteUrl.add("mp.qq.com");
        timWhiteUrl.add("yundong.qq.com");
        tim.setWhiteUrl(qqWhiteUrl);
        tim.setIsUse(true);
        tim.setIsUserBuild(false);
        apps.add(tim);
        size = apps.size();
        return apps;
    }
...
//        }
        ...
        for (App newApp : newAppList) {
            boolean isAdd = false;
            for (App userApp : appList) {
                if (userApp.getPackageName().equals(newApp.getPackageName())) {
                    if (userApp.getRules() == null) {
                        userApp.setRules(newApp.getRules());
                    } else {
                        userApp.getRules().addAll(newApp.getRules());
                    }
                    if (userApp.getWhiteUrl() == null) {
                        userApp.setWhiteUrl(newApp.getWhiteUrl());
                    } else {
                        userApp.getWhiteUrl().addAll(newApp.getWhiteUrl());
                    }
                    isAdd = true;
                }
            }
